			...
			switch ( $anal["action"] )
			{
				...
				case "crit":
					...
					foreach ( $q->lookup_queries as $k => $v )
					{
                        ...
                        {
					        $qu = new reportico_module(); //MODIFIED FROM new reportico();
	...
	function xml2query () 
	{
		...
		foreach ( $q as $cogquery )
		{
			...
			foreach ( $crt as $ci )
			{
				...
				$critquery = new reportico_module();  //MODIFIED FROM new reportico();
...
class reportico_xml_writer
{
	...
	function prepare_web_service_file($templatefile, $savefile = false, $instub)
	{
		global $g_project;
		$smarty = new smarty();
	 	$smarty->compile_dir = find_best_location_in_include_path( BASEPATH. '/cache' );
